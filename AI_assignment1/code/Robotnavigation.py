...
#
#FORMULATION: how we set this up as a search problem(also available in my rough work, so check that also if any confusion)
...
#5.successors: for a state, every move that ends inside the grid on a cell that is not a wall (#). moving diagonally is not allowed
#6.cost: every move costs 1, so the total cost is the number of moves
#7.heuristic (for A*): Manhattan distance to the goal which is (rows dis) + (columns dis). it never overestimates because the robot cant go diagonal, and walls can only make the real route longer

import search
import util
#the four moves the robot can make: (row change, column change)
MOVES = [('U', -1, 0), ('D', 1, 0), ('L', 0, -1), ('R', 0, 1)] # since we're assuming the cost of each grid box is 1


class RobotNavigation(search.SearchProblem):
    #grid=list of rows where each row is a list of cells like ['S', '.', '#'](ive drawn it out in rough notes for clarity)
    #start and goal are optional (row, col). if we leave them out they are taken from where S and G are written in the grid.
    ...
    #state is just where the robot is i.e(row, col)
    ...
    #Manhattan distance: rows dis + columns dis from the goal
    #the robot cant go diagonal or through walls so it never needs
...
#finds where a symbol(i.e S or G in our case) sits in the grid
...
. . . . . # G
"""
#running the algo to show it works similarly to our dry run in rough work
